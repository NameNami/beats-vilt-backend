...
// --- State ---
const form = ref({
    course_id: props.filters.course_id || (props.courses.length > 0 ? props.courses[0].id : ''),
...
const selectedCourseLabel = computed(() => {
    ...
    if (!course && props.courses.length > 0) {
        const first = props.courses[0];
...
const apexAreaOptions = computed(() => ({
    ...
    xaxis: {
        type: 'category',
        categories: props.attendanceTrend.map(d => String(d.week)),
...
const resetFilters = () => {
    form.value = {
        course_id: props.courses.length > 0 ? props.courses[0].id : '',
        programme_id: '',
        start_date: '',
        end_date: '',
    };
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 bg-white border border-slate-300 rounded-2xl p-4 md:p-6 transition-all">
                    <div class="mb-4">
                        <h2 class="text-sm font-bold text-slate-800 uppercase tracking-widest">Attendance Trend</h2>
                    </div>
