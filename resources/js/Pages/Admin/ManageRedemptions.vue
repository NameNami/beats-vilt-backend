            ...
            <div class="bg-white p-6 rounded-2xl  border border-gray-100 col-span-1 h-fit">
                ...
                <form @submit.prevent="submitReward" class="space-y-4">
                    ...
                    <div class="flex justify-end gap-2 pt-4 border-t border-gray-100 mt-4">
                        <button v-if="isEditingReward" type="button" @click="cancelEditReward" class="bg-gray-100 hover:bg-gray-200 text-slate-700 px-4 py-2 rounded-lg font-bold transition text-sm cursor-pointer">
                            Cancel
                        </button>
                        <button type="submit" :disabled="rewardForm.processing" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-lg font-bold transition disabled:opacity-50 w-full text-sm cursor-pointer">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <tr v-for="reward in rewards" :key="reward.id" class="hover:bg-gray-50/50 transition">
                                ...
                                <td class="px-6 py-4 text-right space-x-3">
                                    <button @click="editReward(reward)" class="text-slate-400 hover:text-indigo-600 transition cursor-pointer">
                                        <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </button>
                                    <button @click="deleteReward(reward.id)" class="text-slate-400 hover:text-rose-600 transition cursor-pointer">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        <tr v-for="redemption in redemptions" :key="redemption.id" class="hover:bg-gray-50/50 transition">
                            ...
                            <td class="px-6 py-4 text-right">
                                <select @change="updateStatus(redemption, $event.target.value)" class="bg-white border-gray-200 rounded-lg text-xs py-1 px-2 focus:ring-indigo-500 focus:border-indigo-500 cursor-pointer">
